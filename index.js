...
class Bank {
    ...
    transaction(accObj) {
        this.accounts = this.accounts.map((acc) => acc.accountNumber === accObj.accountNumber ? accObj : acc);
    }
}
let myBank = new Bank();
...
async function bankService(bank) {
    ...
    while (!exit) {
        ...
        if (service.select === "View Balance") {
            ...
            let account = myBank.accounts.find((acc) => acc.accountNumber === parseInt(res.num));
            if (!account) {
                ...
                console.log(`Dear ${chalk.bold.green.italic(name?.firstName)} ${chalk.bold.green.italic(name?.lastName)}, Your Account Balance is ${chalk.bold.italic.blue(`$${account.balance}`)}`);
            }
        }
        // Cash Withdraw
